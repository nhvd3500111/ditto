...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes,fp16):
        ...
        self.fp16=fp16
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True,dropout=0.20,bidirectional=True)

        #2*hidden size due to biderictionality of the LSTM
    ...
    def forward(self, x):
        ...
        if self.fp16:
            ...
            c0 = torch.zeros(2*self.num_layers, x.size(0), self.hidden_size,dtype=torch.float16).to('cuda')
        else:
        ...
        #since we have a biderictional lstm, hidden and cell states should have the double dimension of the lstm layers as the first dimension
        #these are the hidden[0] and hidden[1] sizes. Morevoer there will be half precision floats since we use fp 16 optimization
